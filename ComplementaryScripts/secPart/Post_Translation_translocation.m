...
reaction{2}.eq= sprintf('%s_translocate_1[c] + ATP[c] + H2O[c] => %s_translocate_2[c] + ADP[c] + H+[c] + phosphate[c]',peptide,peptide);
reaction{3}.eq= sprintf('%s_translocate_2[c] => %s_translocate_3[c]',peptide,peptide);
reaction{4}.eq= sprintf('%s_translocate_3[c] + %.15f ATP[c] + %.15f H2O[c] => %s[er] + %.15f ADP[c] + %.15f H+[c] + %d phosphate[c]',peptide,Length,Length,peptide,Length,Length,Length);
end
